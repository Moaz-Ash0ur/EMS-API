...
namespace EMS.BLL.Services
{
    public class UserService : IUserService
    {
        ...
        public UserService(UserManager<ApplicationUser> userManager, IConfiguration config, IEmailService emailSender, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
        ...
        private async Task SendEmailConfirmationAsync(ApplicationUser user)
        {
            ...
            var confirmationLink = $"https://localhost:7003/api/auth/confirm-email?userId={user.Id}&token={Uri.EscapeDataString(token)}";

            await _emailSender.SendEmailAsync(
                user.FullName,
                user.Email,
                "Confirm your email",
                $"<h3>Welcome {user.FullName}</h3><p>Please confirm your email by clicking <a href='{confirmationLink}'>here</a></p>"
            );
        }
        ...
        public async Task<UserResultDto> ForgotPasswordAsync(string email)
        {
            ...
            await _emailSender.SendEmailAsync(user.FullName, email, "Reset Password",
